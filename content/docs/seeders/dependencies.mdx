...
Implement the optional `DependentSeeder` interface on your seeder to declare which seeders must run first:
...
_ = reg.Register("PostSeeder", &seeders.PostSeeder{})
_ = reg.Register("RoleSeeder", &seeders.RoleSeeder{})
_ = reg.Register("UserSeeder", &seeders.UserSeeder{})
...
<Callout type="warn">
  Circular dependencies (e.g., A depends on B, B depends on A) cause the runner to return an error wrapping `seeder.ErrCircularDependency`, with a description of the offending cycle. Design your seeders to form a directed acyclic graph (DAG).
</Callout>
