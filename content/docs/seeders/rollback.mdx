...
<Callout type="info">
  New in v1.3.0: The `RollbackableSeeder` interface, `runner.Rollback()`, `runner.Truncate()`, and the `db:seed:rollback` / `db:seed:truncate` CLI commands are all introduced in v1.3.0.
</Callout>
