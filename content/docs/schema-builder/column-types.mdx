...
|--------|-------------|------------|
| `bp.ID()` | Auto-incrementing unsigned big integer primary key named `id` | _none_ |
...
| `bp.Timestamps()` | Adds nullable `created_at` and `updated_at` columns | _none_ |
| `bp.SoftDeletes()` | Adds a nullable `deleted_at` column | _none_ |
...
`ID` creates an auto-incrementing unsigned big integer column named `id` and sets it as the primary key. It takes no arguments. This is the recommended way to define primary keys.
...
`Timestamps` and `SoftDeletes` are convenience helpers that add common columns. Unlike the other column methods, they return nothing — they add columns directly to the Blueprint:

```go
// Adds nullable created_at and updated_at columns
bp.Timestamps()

// Adds a nullable deleted_at column for soft-delete patterns
bp.SoftDeletes()
```
