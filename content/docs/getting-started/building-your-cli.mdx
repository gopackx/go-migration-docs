...
func main() {
    ...
    // Seeders are registered in a Registry, which is passed to NewRunner
    // along with the *sql.DB and an optional logger (nil = no logging).
    registry := seeder.NewRegistry()
    if err := registry.Register("UserSeeder", &seeders.UserSeeder{}); err != nil {
        log.Fatal(err)
    }
    runner := seeder.NewRunner(registry, db, nil)
