...
You can also register migrations explicitly using `m.Register()`. `Register` returns an `error` — it wraps `ErrInvalidMigrationName` for a bad name format and `ErrDuplicateMigration` if the name is already registered:

```go title="main.go"
m := migrator.New(db)

if err := m.Register("2025_01_15_143022_1234_create_users_table", &migrations.CreateUsersTable{}); err != nil {
    log.Fatal(err)
}
if err := m.Register("2025_01_15_143045_5678_create_posts_table", &migrations.CreatePostsTable{}); err != nil {
    log.Fatal(err)
}
...
if err := m.Register("2025_03_01_120000_9876_legacy_migration", &migrations.LegacyMigration{}); err != nil {
    log.Fatal(err)
}
```
...
`Register()` and `AutoDiscover()` wrap exported sentinel errors that you can inspect with `errors.Is`:

| Error | Returned when |
|-------|---------------|
| `migrator.ErrInvalidMigrationName` | The name does not match the required timestamp-prefixed format |
| `migrator.ErrDuplicateMigration` | A migration with the same name is already registered |

```go
if err := m.Register("bad-name", &migrations.CreateUsersTable{}); err != nil {
    if errors.Is(err, migrator.ErrInvalidMigrationName) {
        // handle invalid name
    }
}
```

`AutoRegister()` panics on these same conditions instead of returning an error, because it runs inside `init()`.
