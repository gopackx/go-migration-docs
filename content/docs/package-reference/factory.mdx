...
**Import path:** `github.com/gopackx/go-migration/pkg/seeder/factory`
...
Returns a new factory copy that uses the provided `Faker` instead of the default time-seeded one. Useful for deterministic, reproducible output.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `faker` | `Faker` | The Faker the returned factory should use |

**Returns:** `*Factory[T]` — a new factory copy (the original is unchanged).

```go
// Deterministic data: same seed always produces the same values
seeded := UserFactory.WithFaker(factory.NewFaker(42))
user := seeded.Make()
```
...
By default, `NewFactory` seeds a `DefaultFaker` from the current time. To control the random source — for reproducible test data — construct a `Faker` explicitly and pass it via `WithFaker`.
...
|-------------|-------------|
| `factory.NewFaker(seed int64)` | Creates a `DefaultFaker` seeded with a fixed value for reproducibility |
| `factory.NewFakerWithRand(rng *math/rand.Rand)` | Creates a `DefaultFaker` backed by a caller-provided `*math/rand.Rand` |
...
f := factory.NewFaker(42)

// Or supply your own rand source
f2 := factory.NewFakerWithRand(mathrand.New(mathrand.NewSource(1)))
```

<Callout type="info">
  `UUID`, `Date`, and `DateBetween` draw from `crypto/rand` when available, so those values stay random even with a fixed seed.
</Callout>
...
| Factory | `WithFaker` | `WithFaker(faker) *Factory[T]` | Use a custom Faker |
| Faker | `NewFaker` | `NewFaker(seed int64) *DefaultFaker` | Seeded Faker constructor |
| Faker | `NewFakerWithRand` | `NewFakerWithRand(rng) *DefaultFaker` | Faker from a `*rand.Rand` |
