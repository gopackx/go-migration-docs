...
const docsCards = [
  { label: "01 · INTRODUCTION", title: "Getting Started", description: "Install go-migration, scaffold your first struct migration, and run it in under five minutes.", href: "/docs" },
  { label: "02 · SCHEMA", title: "Schema Builder Reference", description: "Every column type, index, foreign key, and constraint the fluent API supports — with examples.", href: "/docs/schema-builder/creating-tables" },
  { label: "03 · MIGRATIONS", title: "Writing Migrations", description: "Patterns for Up / Down methods, safe production rollouts, and version-controlled schema evolution.", href: "/docs/migrations/defining-migrations" },
  { label: "04 · DATA", title: "Seeders & Factories", description: "Build deterministic or faker-driven test data with factories, traits, and dependency resolution.", href: "/docs/seeders/defining-seeders" },
  { label: "05 · CLI", title: "CLI Command Reference", description: "Flags, environments, and output for migrate, rollback, fresh, status, seed, and scaffold commands.", href: "/docs/cli" },
  { label: "06 · COOKBOOK", title: "Recipes & Patterns", description: "Real-world patterns: zero-downtime migrations, multi-tenant schemas, testing strategies, and more.", href: "/docs" },
];

const databases = [
  { mark: "PG", name: "PostgreSQL", desc: "9.6 → latest · JSON, arrays, CTE", markColor: "text-[#60A5FA] bg-[#60A5FA]/15" },
  { mark: "My", name: "MySQL / MariaDB", desc: "5.7 → latest · InnoDB tuned", markColor: "text-[#F59E0B] bg-[#F59E0B]/15" },
  { mark: "SL", name: "SQLite", desc: "3.8+ · Ideal for tests & embedded", markColor: "text-[#22C55E] bg-[#22C55E]/15" },
  { mark: "MS", name: "SQL Server", desc: "2017+ · Windows & Linux drivers", markColor: "text-[#14B8A6] bg-[#14B8A6]/15" },
];
...
function CopyButton({ text, label = "Copy" }: { text: string; label?: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11px] font-medium text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200 transition-colors"
    ...
    <main className="w-full">
      ...
      <section className="mx-auto w-full max-w-[1440px] px-5 pt-10 pb-12 md:px-16 md:pt-24 md:pb-24 lg:px-[120px]">
        <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          {/* Code card */}
          <div className="overflow-hidden rounded-2xl border border-fd-border bg-[#0A0A0A] shadow-[0_20px_48px_-8px_rgba(168,85,247,0.12),_0_8px_24px_-6px_rgba(0,0,0,0.08)]">
            <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-3">
              ...
              <span className="font-mono text-[11px] text-zinc-500 sm:text-[12px]">migrations/001_create_users.go</span>
              <CopyButton
                text={`package migrations\n\ntype CreateUsers struct{}\n\nfunc (m *CreateUsers) Up(s *schema.Builder) {\n    s.Create("users", func(t *schema.Table) {\n        t.ID()\n        t.String("email").Unique()\n        t.String("name").Nullable()\n        t.Timestamps()\n    })\n}`}
              />
            </div>
            <pre className="overflow-x-auto px-5 py-6 font-mono text-[12px] leading-[1.7] text-zinc-200 sm:px-7 sm:text-[13px]">
{`package migrations
...
func (m *CreateUsers) Up(s *schema.Builder) {
    s.Create("users", func(t *schema.Table) {
`}
<span className="text-[#14B8A6]">{`        t.ID()
        t.String("email").Unique()
        t.String("name").Nullable()
        t.Timestamps()
`}</span>
{`    })
      ...
      <section className="border-t border-fd-border">
        <div className="mx-auto w-full max-w-[1440px] px-5 py-16 md:px-16 md:py-24 lg:px-[120px]">
          ...
          <div className="mt-10 grid gap-5 md:mt-12 md:grid-cols-2 lg:grid-cols-3">
            {docsCards.map((c) => (
              <Link
                key={c.label}
                href={c.href}
                className="group flex flex-col gap-4 rounded-lg border border-fd-border bg-fd-secondary/50 p-7 no-underline transition-colors hover:border-[#14B8A6]/30 hover:bg-fd-accent/30"
              >
                <span className="font-mono text-[11px] font-semibold tracking-[0.12em] text-[#14B8A6]">{c.label}</span>
                <h3 className="text-[20px] font-bold text-fd-foreground">{c.title}</h3>
                <p className="text-[14px] leading-[1.55] text-fd-muted-foreground">{c.description}</p>
                <span className="mt-auto inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#14B8A6]">
                  Read more
                  <ArrowRight className="h-3.5 w-3.5" />
                </span>
              </Link>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {databases.map((db) => (
              <div key={db.name} className="flex flex-col gap-2.5 rounded-lg border border-fd-border bg-fd-secondary/50 p-6">
      ...
      <section className="border-t border-fd-border">
        <div className="mx-auto w-full max-w-[1440px] px-5 py-16 md:px-16 md:py-24 lg:px-[120px]">
          <div className="flex flex-col items-center gap-6 rounded-lg bg-[#14B8A6] px-6 py-14 text-center shadow-[0_4px_12px_rgba(0,0,0,0.04),_0_1px_3px_rgba(0,0,0,0.03)] md:px-20 md:py-16">
            <span className="inline-flex items-center gap-2 rounded-full bg-white/20 px-3.5 py-1.5 text-[11px] font-semibold tracking-[0.12em] text-[#0A0A0A]">
              <span className="h-2 w-2 rounded-full bg-white" />
              OPEN SOURCE · MIT LICENSE
            </span>
            <h2 className="max-w-3xl text-[36px] font-bold leading-[1.1] tracking-[-0.02em] text-[#0A0A0A] md:text-[52px]">
              Ready to ship faster migrations?
            </h2>
            <p className="max-w-[720px] text-[15px] leading-[1.5] text-[#0A0A0A]/80 md:text-[17px]">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              ...
              <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-black/20 bg-white/15 px-6 py-3.5 text-[15px] font-semibold text-[#0A0A0A] backdrop-blur transition-colors hover:bg-white/25">
        ...
        <div className="mx-auto w-full max-w-[1440px] px-5 py-12 md:px-16 md:py-16 lg:px-[120px]">
          <div className="grid gap-10 lg:grid-cols-[1.5fr_2fr]">
            <div className="flex flex-col gap-4">
              ...
              <div className="inline-flex w-fit items-center gap-2 rounded border border-fd-border bg-fd-secondary px-3 py-2 font-mono text-[12px]">
                <span className="text-[#14B8A6]">$</span>
                <span className="text-fd-foreground">{GO_GET_CMD}</span>
              </div>
            </div>
